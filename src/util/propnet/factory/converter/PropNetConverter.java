...
public final class PropNetConverter
{
	...
	private Proposition convertConjunct(GdlLiteral literal)
	{
		...
		else
		{
			...
			Proposition proposition = (sentence.getName().getValue().equalsIgnoreCase("true")) ? getProposition(sentence.get(0)) : getProposition(sentence.toTerm());
			components.add(proposition);
	...
	{
		if ( sentence.getName().getValue().equalsIgnoreCase("next") )
		{
	...
	{
		if ( sentence.getName().getValue().equalsIgnoreCase("init") )
		{
